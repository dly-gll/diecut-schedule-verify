...
// V5.1.2-WORKFLOW-BOARD-SNAPSHOT-FIRST-VERIFIED
...
function parseTextDateFromString(text, _keywordList = []) {
...
function updateWorkflowTransition(order, stage, snapshotDate, expectedDate=null, statusText='') {
  ...
  if (stage !== prev) {
    const to = workflowStageRank(stage);
    ...
    const rows=db.prepare(`
      ...
      WHERE snap.batch_id=? AND snap.stage=?
        AND (
          ...
          OR snap.id=(SELECT MAX(s2.id) FROM workflow_snapshots s2 WHERE s2.batch_id=snap.batch_id AND s2.work_order_number=snap.work_order_number)
        )
      ORDER BY
...
app.get('/api/workflow/kpi', requireAuth, (req,res)=>{
  ...
  }catch{res.status(500).json({success:false,message:'读取四板块KPI失败'});}
});
...
app.get('/api/workflow/import-history', requireAuth, (req,res)=>{
  ...
  }catch{res.status(500).json({success:false,message:'读取导入历史失败'});}
});
...
function autoNormalizeImportedOrder(row, index, productMap) {
  ...
  const deliveryTime = normalizeImportedDate(findImportValue(row, [
