...
const baseRef = 'c401b9c37dd8e29f359d8ff03aec591aac9a78b4';
...
// Always rebuild the workflow UI from the last known-good release UI.
// This intentionally discards only the experimental workflow-board UI patch,
// while preserving the rest of the known-good application.
...
for (const [label,id] of [['欠料','workflow-count-shortage'],['有料待发','workflow-count-available_to_issue'],['车间待排','workflow-count-waiting_schedule'],['车间在制','workflow-count-in_process']]) {
  source = replaceOnce(source, `<span>${label}</span>`, `<span>${label} <b class="workflow-subnav-count" id="${id}">0</b></span>`, `${label} badge`);
}

source = replaceOnce(source, 'let workflowKpiData = null;',
