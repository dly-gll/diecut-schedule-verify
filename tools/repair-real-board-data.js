...
if (!source.includes('// V5.1.5-WORKFLOW-LEGACY-FIELD-BACKFILL')) {
  ...
  const insert = `backfillOrderShippingQuantities();
...
function backfillWorkflowLegacyBoardFields() {
  ...
  const rows = db.prepare(
    `SELECT id, work_order_number, quantity, shipping_quantity, shipping_required_date, delivery_date, raw_json, sheet_name
     FROM workflow_snapshots
     WHERE batch_id=? AND work_order_number IS NOT NULL AND TRIM(work_order_number)<>''
       AND (sheet_name LIKE '%在制工单明细%' OR sheet_name LIKE '%生产工单明细%')`
  ).all(latestBatch.id);
  const update = db.prepare(`
    UPDATE workflow_snapshots
    SET quantity=?, shipping_quantity=?, shipping_required_date=?, delivery_date=?, raw_json=?
    WHERE id=?
  `);
  let changed = 0;
  const tx = db.transaction(() => {
    for (const row of rows) {
      ...
      const legacyQuantity = numberOr(fields[8], NaN);       // 预计产量
      const legacyShipping = numberOr(fields[14], NaN);     // 出货数量
      const legacyShipDate = normalizeImportedDate(fields[15]); // 要求出货日期
      const quantity = Number(row.quantity) > 0 ? Number(row.quantity)
        : Number.isFinite(legacyQuantity) && legacyQuantity > 0 ? legacyQuantity
        : Number(row.quantity || 0);
      const shippingQuantity = Number(row.shipping_quantity) > 0 ? Number(row.shipping_quantity)
        : Number.isFinite(Number(raw.shipping_quantity)) && Number(raw.shipping_quantity) >= 0 ? Number(raw.shipping_quantity)
        : Number.isFinite(Number(raw.delivery_qty)) && Number(raw.delivery_qty) >= 0 ? Number(raw.delivery_qty)
        : Number.isFinite(legacyShipping) && legacyShipping >= 0 ? legacyShipping
        : Number(row.shipping_quantity || 0);
      ...
      if (quantity !== Number(row.quantity || 0)
        || shippingQuantity !== Number(row.shipping_quantity || 0)
        || shippingRequiredDate !== (row.shipping_required_date || null)
